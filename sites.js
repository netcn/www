window.sitesData = {
  zh: {
    "搜索": [
      {name:"百度",url:"https://www.baidu.com/"},
      {name:"必应",url:"https://cn.bing.com/"},
      {name:"搜狗",url:"https://www.sogou.com/"},
      {name:"360搜",url:"https://www.so.com/"},
      {name:"谷歌",url:"https://www.google.cn/"},
      {name:"神马",url:"https://www.sm.cn/"},
      {name:"搜搜",url:"https://www.soso.com/"}
    ],
    "智能": [
      {name:"推演",url:"https://specul.com/"},
      {name:"DeepSeek",url:"https://www.deepseek.com/"},
      {name:"豆包",url:"https://www.doubao.com/"},
      {name:"Qwen",url:"https://chat.qwen.ai/"},
      {name:"千问",url:"https://www.qianwen.com/"},	  
      {name:"Kimi",url:"https://www.kimi.com/"},
      {name:"清言",url:"https://chatglm.cn/"},	  
      {name:"文心",url:"https://chat.baidu.com/"},	  
      {name:"ChatGPT",url:"https://chat.openai.com/"},
      {name:"Claude",url:"https://claude.ai/"},
      {name:"Gemini",url:"https://gemini.google.com/"}
    ],
    "视频": [
      {name:"抖音",url:"https://www.douyin.com/"},
      {name:"快手",url:"https://www.kuaishou.com/"},
      {name:"哔哩哔哩",url:"https://www.bilibili.com/"},
      {name:"腾讯视频",url:"https://v.qq.com/"},
      {name:"爱奇艺",url:"https://www.iqiyi.com/"},
      {name:"优酷",url:"https://www.youku.com/"},
      {name:"西瓜",url:"https://www.ixigua.com/"},
      {name:"芒果TV",url:"https://www.mgtv.com/"},
      {name:"咪咕",url:"https://www.miguvideo.com/"},
      {name:"看电视",url:"https://tvcn.org/"}
    ],
    "社交": [
      {name:"微信",url:"https://weixin.qq.com/"},
      {name:"微博",url:"https://weibo.com/"},
      {name:"小红书",url:"https://www.xiaohongshu.com/"}
    ],
    "电商": [
      {name:"淘宝",url:"https://www.taobao.com/"},
      {name:"拼多多",url:"https://www.pinduoduo.com/"},
      {name:"京东",url:"https://www.jd.com/"},
      {name:"天猫",url:"https://www.tmall.com/"},
      {name:"1688",url:"https://www.1688.com/"},
      {name:"苏宁",url:"https://www.suning.com/"},
      {name:"得物",url:"https://www.dewu.com/"},
      {name:"当当",url:"https://www.dangdang.com/"},
      {name:"什么值得买",url:"https://www.smzdm.com/"}
    ],
    "出行": [
      {name:"高德地图",url:"https://www.amap.com/"},
      {name:"百度地图",url:"https://map.baidu.com/"},
      {name:"12306",url:"https://www.12306.cn/"},
      {name:"公交",url:"https://www.8684.cn/"},
      {name:"天气",url:"http://www.weather.com.cn/"},
      {name:"大众点评",url:"https://www.dianping.com/"}
    ],
    "资讯": [
      {name:"今日头条",url:"https://www.toutiao.com/"},
      {name:"腾讯新闻",url:"https://news.qq.com/"},
      {name:"网易新闻",url:"https://news.163.com/"},
      {name:"搜狐新闻",url:"https://news.sohu.com/"},
      {name:"新浪新闻",url:"https://news.sina.com.cn/"},
      {name:"澎湃",url:"https://www.thepaper.cn/"},
      {name:"观察者网",url:"https://www.guancha.cn/"},
      {name:"凤凰军事",url:"https://mil.ifeng.com/"}
    ],
    "门户": [
      {name:"腾讯",url:"https://www.qq.com/"},
      {name:"新浪",url:"https://www.sina.com.cn/"},
      {name:"网易",url:"https://www.163.com/"},
      {name:"搜狐",url:"https://www.sohu.com/"}
    ],
    "官媒": [
      {name:"新华",url:"https://www.xinhuanet.com/"},
      {name:"人民",url:"https://www.people.com.cn/"},
      {name:"央视",url:"https://www.cctv.com/"},
      {name:"中国日报",url:"https://www.chinadaily.com.cn/"},
      {name:"光明",url:"https://www.gmw.cn/"},
      {name:"中新网",url:"https://www.chinanews.com.cn/"},
      {name:"中国网",url:"https://www.china.com.cn/"},
      {name:"央广网",url:"https://www.cnr.cn/"},
      {name:"国际在线",url:"https://www.cri.cn/"}
    ],
    "时政": [
      {name:"环球",url:"https://www.huanqiu.com/"},
      {name:"参考消息",url:"https://www.cankaoxiaoxi.com/"},
      {name:"新京报",url:"https://www.bjnews.com.cn/"},
      {name:"南方周末",url:"https://www.infzm.com/"},
      {name:"中华军事",url:"https://military.china.com/"},
      {name:"环球军事",url:"https://mil.huanqiu.com/"},
      {name:"新浪军事",url:"https://mil.news.sina.com.cn/"}
    ],
    "财经": [
      {name:"财联社",url:"https://www.cls.cn/"},
      {name:"华尔街见闻",url:"https://wallstreetcn.com/"},
      {name:"第一财经",url:"https://www.yicai.com/"},
      {name:"财新",url:"https://www.caixin.com/"},
      {name:"界面",url:"https://www.jiemian.com/"},
      {name:"每日经济新闻",url:"https://www.nbd.com.cn/"},
      {name:"21经济",url:"https://www.21jingji.com/"},
      {name:"经济观察报",url:"https://www.eeo.com.cn/"},
      {name:"投资界",url:"https://www.pedaily.cn/"},
      {name:"51投资导航",url:"https://51invest.com/"}
    ],
    "科技": [
      {name:"36氪",url:"https://www.36kr.com/"},
      {name:"虎嗅",url:"https://www.huxiu.com/"},
      {name:"极客公园",url:"https://www.geekpark.net/"},
      {name:"钛媒体",url:"https://www.tmtpost.com/"},
      {name:"品玩",url:"https://www.pingwest.com/"},
      {name:"爱范儿",url:"https://www.ifanr.com/"},
      {name:"量子位",url:"https://www.qbitai.com/"},
      {name:"亿欧",url:"https://www.iyiou.com/"},
      {name:"DoNews",url:"https://www.donews.com/"},
      {name:"爱分析",url:"https://www.ifenxi.com/"}
    ],
    "数码": [
      {name:"IT之家",url:"https://www.ithome.com/"},
      {name:"中关村在线",url:"https://www.zol.com.cn/"},
      {name:"快科技",url:"https://www.mydrivers.com/"},
      {name:"太平洋电脑网",url:"https://www.pconline.com.cn/"},
      {name:"ZAKER",url:"https://www.myzaker.com/"},
      {name:"天极网",url:"https://www.yesky.com/"}
    ],
    "极客": [
      {name:"少数派",url:"https://sspai.com/"},
      {name:"果壳",url:"https://www.guokr.com/"},
      {name:"站长之家",url:"https://www.chinaz.com/"},
      {name:"雷锋网",url:"https://www.leiphone.com/"},
      {name:"动点科技",url:"https://cn.technode.com/"},
      {name:"亿邦动力",url:"https://www.ebrun.com/"},
      {name:"电子工程世界",url:"https://www.eeworld.com.cn/"},
      {name:"IT168",url:"https://www.it168.com/"},
      {name:"甲子光年",url:"https://www.jazzyear.com/"}
    ],
    "编程": [
      {name:"GitHub",url:"https://github.com/"},
      {name:"CSDN",url:"https://www.csdn.net/"},
      {name:"掘金",url:"https://juejin.cn/"},
      {name:"开源中国",url:"https://www.oschina.net/"},
      {name:"力扣",url:"https://leetcode.cn/"},
      {name:"牛客网",url:"https://www.nowcoder.com/"}
    ],
    "户外": [
      {name:"两步路",url:"https://www.2bulu.com/"},
      {name:"入野",url:"https://ruhike.com/"},
      {name:"大徒步",url:"https://datubu.com/"},
      {name:"跑起来",url:"https://paobu.org/"},
      {name:"国保地图",url:"https://guobao.org/"}
    ],
    "社区": [
      {name:"StackOverflow",url:"https://stackoverflow.com/"},
      {name:"博客园",url:"https://www.cnblogs.com/"},
      {name:"V2EX",url:"https://www.v2ex.com/"},
      {name:"51CTO",url:"https://www.51cto.com/"},
      {name:"SegmentFault",url:"https://segmentfault.com/"},
      {name:"InfoQ",url:"https://www.infoq.cn/"}
    ],
    "音乐": [
      {name:"QQ音乐",url:"https://y.qq.com/"},
      {name:"网易云音乐",url:"https://music.163.com/"},
      {name:"酷狗",url:"https://www.kugou.com/"},
      {name:"酷我",url:"https://www.kuwo.cn/"},
      {name:"咪咕音乐",url:"https://music.migu.cn/"},
      {name:"MIDI音乐",url:"https://midicn.com/"},
      {name:"音乐人",url:"https://midi.cn/"}
    ],
    "论坛": [
      {name:"知乎",url:"https://www.zhihu.com/"},
      {name:"贴吧",url:"https://tieba.baidu.com/"},
      {name:"豆瓣",url:"https://www.douban.com/"},
      {name:"虎扑",url:"https://www.hupu.com/"},
      {name:"简书",url:"https://www.jianshu.com/"},
      {name:"天涯",url:"https://www.tianya.cn/"},
      {name:"即刻",url:"https://www.okjike.com/"},
      {name:"皮皮虾",url:"https://www.pipix.com/"},
      {name:"最右",url:"https://www.izuiyou.com/"},
      {name:"人人",url:"https://www.renren.com/"},
      {name:"LOFTER",url:"https://www.lofter.com/"},
      {name:"悟空问答",url:"https://www.wukong.com/"}
    ],
    "游戏": [
      {name:"Steam",url:"https://store.steampowered.com/"},
      {name:"4399",url:"https://www.4399.com/"},
      {name:"腾讯游戏",url:"https://game.qq.com/"},
      {name:"网易游戏",url:"https://game.163.com/"},
      {name:"米哈游",url:"https://www.mihoyo.com/"},
      {name:"TapTap",url:"https://www.taptap.cn/"},
      {name:"九游",url:"https://www.9game.cn/"},
      {name:"好游快爆",url:"https://www.3839.com/"},
      {name:"当乐",url:"https://www.d.cn/"},
      {name:"电玩巴士",url:"https://www.tgbus.com/"}
    ],
    "游媒": [
      {name:"游民星空",url:"https://www.gamersky.com/"},
      {name:"3DM",url:"https://www.3dmgame.com/"},
      {name:"游侠",url:"https://www.ali213.net/"},
      {name:"17173",url:"https://www.17173.com/"},
      {name:"多玩",url:"https://www.duowan.com/"},
      {name:"逗游",url:"https://www.doyo.cn/"},
      {name:"52pk",url:"https://www.52pk.com/"},
      {name:"178",url:"https://www.178.com/"},
      {name:"游迅",url:"https://www.yxdown.com/"}
    ],
    "直播": [
      {name:"斗鱼",url:"https://www.douyu.com/"},
      {name:"虎牙",url:"https://www.huya.com/"},
      {name:"YY",url:"https://www.yy.com/"},
      {name:"花椒",url:"https://www.huajiao.com/"},
      {name:"映客",url:"https://www.inke.cn/"},
      {name:"酷狗直播",url:"https://fanxing.kugou.com/"},
      {name:"战旗",url:"https://www.zhanqi.tv/"},
      {name:"直播吧",url:"https://www.zhibo8.com/"}
    ],
    "影视": [
      {name:"猫眼电影",url:"https://www.maoyan.com/"},
      {name:"豆瓣电影",url:"https://movie.douban.com/"},
      {name:"乐视",url:"https://www.le.com/"},
      {name:"央视频",url:"https://www.yangshipin.cn/"},
      {name:"风行",url:"https://www.fun.tv/"},
      {name:"华数TV",url:"https://www.wasu.cn/"}
    ],
    "影音": [
      {name:"AcFun",url:"https://www.acfun.cn/"},
      {name:"PP视频",url:"https://www.pptv.com/"},
      {name:"搜狐视频",url:"https://tv.sohu.com/"},
      {name:"央视影音",url:"https://app.cctv.com/"}
    ],
    "播客": [
      {name:"喜马拉雅",url:"https://www.ximalaya.com/"},
      {name:"蜻蜓FM",url:"https://www.qingting.fm/"},
      {name:"荔枝FM",url:"https://www.lizhi.fm/"},
      {name:"懒人听书",url:"https://www.lrts.me/"}
    ],
    "体育": [
      {name:"腾讯体育",url:"https://sports.qq.com/"},
      {name:"新浪体育",url:"https://sports.sina.com.cn/"},
      {name:"PP体育",url:"https://www.ppsport.com/"},
      {name:"央视体育",url:"https://sports.cctv.com/"},
      {name:"网易体育",url:"https://sports.163.com/"},
      {name:"搜狐体育",url:"https://sports.sohu.com/"}
    ],
    "阅读": [
      {name:"起点",url:"https://www.qidian.com/"},
      {name:"微信读书",url:"https://weread.qq.com/"},
      {name:"番茄小说",url:"https://www.fanqienovel.com/"},
      {name:"晋江",url:"https://www.jjwxc.net/"},
      {name:"掌阅",url:"https://www.ireader.com/"},
      {name:"书旗小说",url:"https://www.shuqi.com/"},
      {name:"豆瓣阅读",url:"https://read.douban.com/"},
      {name:"纵横",url:"https://www.zongheng.com/"},
      {name:"网易云阅读",url:"https://yuedu.163.com/"},
      {name:"实录",url:"https://shilu.org/"},
      {name:"尺度",url:"https://chidu.org/"},
      {name:"孔夫子旧书网",url:"https://www.kongfz.com/"}
    ],
    "百科": [
      {name:"百度百科",url:"https://baike.baidu.com/"},
      {name:"维基百科",url:"https://zh.wikipedia.org/"},
      {name:"知网",url:"https://www.cnki.net/"},
      {name:"海词",url:"https://dict.cn/"},
      {name:"大英百科",url:"https://www.britannica.com/"},
      {name:"汉典",url:"https://www.zdic.net/"},
      {name:"韦氏词典",url:"https://www.merriam-webster.com/"},
      {name:"互联网档案馆",url:"https://archive.org/"},
      {name:"可汗学院",url:"https://www.khanacademy.org/"},
      {name:"Dictionary",url:"https://www.dictionary.com/"}
    ],
    "工具": [
      {name:"VS Code",url:"https://code.visualstudio.com/"},
      {name:"Git",url:"https://git-scm.com/"},
      {name:"Node.js",url:"https://nodejs.org/"},
      {name:"Docker Hub",url:"https://hub.docker.com/"},
      {name:"PyPI",url:"https://pypi.org/"},
      {name:"Mozilla",url:"https://www.mozilla.org/"},
      {name:"SourceForge",url:"https://sourceforge.net/"},
      {name:"Notepad++",url:"https://notepad-plus-plus.org/"},
      {name:"Sublime",url:"https://www.sublimetext.com/"},
      {name:"图床",url:"https://sm.ms/"}
    ],
    "模型": [
      {name:"讯飞星火",url:"https://xinghuo.xfyun.cn/"},
      {name:"秘塔AI",url:"https://metaso.cn/"},
      {name:"天工AI",url:"https://www.tiangong.cn/"},
      {name:"可灵AI",url:"https://klingai.com/"},
      {name:"百川智能",url:"https://www.baichuan-ai.com/"}
    ],
    "企业": [
      {name:"商汤",url:"https://www.sensetime.com/"},
      {name:"旷视",url:"https://www.megvii.com/"},
      {name:"腾讯AI",url:"https://ai.tencent.com/"},
      {name:"第四范式",url:"https://www.4paradigm.com/"},
      {name:"云从",url:"https://www.cloudwalk.com/"}
    ],
    "云服": [
      {name:"阿里云",url:"https://www.aliyun.com/"},
      {name:"腾讯云",url:"https://cloud.tencent.com/"},
      {name:"华为云",url:"https://www.huaweicloud.com/"},
      {name:"蓝奏云",url:"https://www.lanzou.com/"},
      {name:"123云盘",url:"https://www.123pan.com/"},
      {name:"又拍",url:"https://x.yupoo.com/"}
    ],
    "网盘": [
      {name:"百度网盘",url:"https://pan.baidu.com/"},
      {name:"阿里云盘",url:"https://www.aliyundrive.com/"},
      {name:"WPS",url:"https://www.wps.cn/"},
      {name:"腾讯文档",url:"https://docs.qq.com/"},
      {name:"石墨文档",url:"https://shimo.im/"},
      {name:"115",url:"https://www.115.com/"},
      {name:"幕布",url:"https://mubu.com/"},
      {name:"ProcessOn",url:"https://www.processon.com/"}
    ],
    "办公": [
      {name:"钉钉",url:"https://www.dingtalk.com/"},
      {name:"飞书",url:"https://www.feishu.cn/"},
      {name:"企业微信",url:"https://work.weixin.qq.com/"},
      {name:"腾讯会议",url:"https://meeting.tencent.com/"},
      {name:"Skype",url:"https://www.skype.com/"}
    ],
    "笔记": [
      {name:"有道",url:"https://www.youdao.com/"},
      {name:"印象笔记",url:"https://www.yinxiang.com/"},
      {name:"有道翻译",url:"https://fanyi.youdao.com/"},
      {name:"DeepL",url:"https://www.deepl.com/"}
    ],
    "邮箱": [
      {name:"QQ邮箱",url:"https://mail.qq.com/"},
      {name:"163邮箱",url:"https://mail.163.com/"},
      {name:"126邮箱",url:"https://www.126.com/"},
      {name:"Outlook",url:"https://outlook.live.com/"},
      {name:"Gmail",url:"https://mail.google.com/"},
      {name:"新浪邮箱",url:"https://mail.sina.com.cn/"},
      {name:"企业邮箱",url:"https://www.exmail.qq.com/"}
    ],
    "金融": [
      {name:"支付宝",url:"https://www.alipay.com/"},
      {name:"度小满",url:"https://www.duxiaoman.com/"},
      {name:"京东金融",url:"https://jr.jd.com/"},
      {name:"陆金所",url:"https://www.lu.com/"},
      {name:"PayPal",url:"https://www.paypal.com/"}
    ],
    "证券": [
      {name:"东方财富",url:"https://www.eastmoney.com/"},
      {name:"同花顺",url:"https://www.10jqka.com.cn/"},
      {name:"雪球",url:"https://xueqiu.com/"},
      {name:"和讯",url:"https://www.hexun.com/"},
      {name:"金十数据",url:"https://www.jin10.com/"},
      {name:"金融界",url:"https://www.jrj.com.cn/"},
      {name:"证券之星",url:"https://www.stockstar.com/"},
      {name:"中财网",url:"https://www.cfi.net.cn/"}
    ],
    "投资": [
      {name:"天天基金",url:"https://www.1234567.com.cn/"},
      {name:"华泰证券",url:"https://www.htsc.com/"},
      {name:"中信证券",url:"https://www.cs.ecitic.com/"},
      {name:"广发证券",url:"https://www.gf.com.cn/"},
      {name:"随手记",url:"https://www.sui.com/"},
      {name:"挖财",url:"https://www.wacai.com/"}
    ],
    "银行": [
      {name:"工商银行",url:"https://www.icbc.com.cn/"},
      {name:"建设银行",url:"https://www.ccb.com/"},
      {name:"农业银行",url:"https://www.abchina.com/"},
      {name:"中国银行",url:"https://www.boc.cn/"},
      {name:"招商银行",url:"https://www.cmbchina.com/"},
      {name:"交通银行",url:"https://www.bankcomm.com/"}
    ],
    "保险": [
      {name:"中国平安",url:"https://www.pingan.com/"},
      {name:"中国人寿",url:"https://www.e-chinalife.com/"},
      {name:"众安保险",url:"https://www.zhongan.com/"}
    ],
    "旅游": [
      {name:"携程",url:"https://www.ctrip.com/"},
      {name:"去哪儿",url:"https://www.qunar.com/"},
      {name:"飞猪",url:"https://www.fliggy.com/"},
      {name:"马蜂窝",url:"https://www.mafengwo.cn/"},
      {name:"同程",url:"https://www.ly.com/"},
      {name:"途牛",url:"https://www.tuniu.com/"},
      {name:"爱彼迎",url:"https://www.airbnb.cn/"},
      {name:"穷游",url:"https://www.qyer.com/"},
      {name:"途家",url:"https://www.tujia.com/"},
      {name:"Booking",url:"https://www.booking.com/"},
      {name:"租租车",url:"https://www.zuzuche.com/"},
      {name:"Agoda",url:"https://www.agoda.com/"},
      {name:"Trip.com",url:"https://www.trip.com/"},
      {name:"Klook",url:"https://www.klook.com/"},
      {name:"木鸟民宿",url:"https://www.muniao.com/"}
    ],
    "租车": [
      {name:"神州租车",url:"https://www.zuche.com/"},
      {name:"一嗨租车",url:"https://www.1hai.cn/"}
    ],
    "航空": [
      {name:"南方航空",url:"https://www.csair.com/"},
      {name:"东方航空",url:"https://www.ceair.com/"},
      {name:"中国国航",url:"https://www.airchina.com.cn/"},
      {name:"海南航空",url:"https://www.hnair.com/"},
      {name:"春秋航空",url:"https://www.ch.com/"},
      {name:"深圳航空",url:"https://www.shenzhenair.com/"},
      {name:"厦门航空",url:"https://www.xiamenair.com/"}
    ],
    "汽车": [
      {name:"比亚迪",url:"https://www.byd.com/"},
      {name:"特斯拉",url:"https://www.tesla.cn/"},
      {name:"汽车之家",url:"https://www.autohome.com.cn/"},
      {name:"懂车帝",url:"https://www.dongchedi.com/"},
      {name:"蔚来",url:"https://www.nio.cn/"},
      {name:"理想汽车",url:"https://www.lixiang.com/"},
      {name:"小鹏汽车",url:"https://www.xiaopeng.com/"}
    ],
    "房产": [
      {name:"链家",url:"https://www.lianjia.com/"},
      {name:"贝壳找房",url:"https://www.ke.com/"},
      {name:"安居客",url:"https://www.anjuke.com/"},
      {name:"房天下",url:"https://www.fang.com/"},
      {name:"自如",url:"https://www.ziroom.com/"},
      {name:"我爱我家",url:"https://www.5i5j.com/"},
      {name:"焦点",url:"https://www.focus.cn/"},
      {name:"土巴兔",url:"https://www.to8to.com/"},
      {name:"齐家",url:"https://www.jia.com/"}
    ],
    "快递": [
      {name:"顺丰",url:"https://www.sf-express.com/"},
      {name:"菜鸟",url:"https://www.cainiao.com/"},
      {name:"中通",url:"https://www.zto.com/"},
      {name:"圆通",url:"https://www.yto.net.cn/"},
      {name:"韵达",url:"https://www.yundaex.com/"},
      {name:"申通",url:"https://www.sto.cn/"},
      {name:"极兔",url:"https://www.jtexpress.com/"},
      {name:"快递100",url:"https://www.kuaidi100.com/"},
      {name:"联邦快递",url:"https://www.fedex.com/"}
    ],
    "物流": [
      {name:"货拉拉",url:"https://www.huolala.cn/"},
      {name:"闪送",url:"https://www.ishansong.com/"},
      {name:"UU跑腿",url:"https://www.uupt.com/"},
      {name:"UPS快递",url:"https://www.ups.com/"}
    ],
    "招聘": [
      {name:"Boss直聘",url:"https://www.zhipin.com/"},
      {name:"前程无忧",url:"https://www.51job.com/"},
      {name:"智联招聘",url:"https://www.zhaopin.com/"},
      {name:"猎聘",url:"https://www.liepin.com/"},
      {name:"领英",url:"https://www.linkedin.com/"},
      {name:"拉勾",url:"https://www.lagou.com/"},
      {name:"应届生",url:"https://www.yingjiesheng.com/"},
      {name:"实习僧",url:"https://www.shixiseng.com/"}
    ],
    "生活": [
      {name:"58同城",url:"https://www.58.com/"},
      {name:"赶集",url:"https://www.ganji.com/"},
      {name:"本地宝",url:"https://www.bendibao.com/"},
      {name:"大麦",url:"https://www.damai.cn/"},
      {name:"百姓",url:"https://www.baixing.com/"},
      {name:"宝宝树",url:"https://www.babytree.com/"},
      {name:"妈妈",url:"https://www.mama.cn/"},
      {name:"天鹅到家",url:"https://www.daojia.com/"},
      {name:"宜家",url:"https://www.ikea.cn/cn/zh/"},
      {name:"Keep",url:"https://www.gotokeep.com/"}
    ],
    "美食": [
      {name:"下厨房",url:"https://www.xiachufang.com/"},
      {name:"美食天下",url:"https://www.meishichina.com/"},
      {name:"美食杰",url:"https://www.meishij.net/"},
      {name:"豆果",url:"https://www.douguo.com/"},
      {name:"好豆",url:"https://www.haodou.com/"},
      {name:"香哈",url:"https://www.xiangha.com/"},
      {name:"茶谱",url:"https://chapu.org/"},
      {name:"喝酒",url:"https://hejiu.org/"},
      {name:"寻味中国",url:"https://wecn.org/"}
    ],
    "健康": [
      {name:"丁香医生",url:"https://www.dxy.com/"},
      {name:"好大夫",url:"https://www.haodf.com/"},
      {name:"39健康",url:"https://www.39.net/"},
      {name:"薄荷健康",url:"https://www.boohee.com/"},
      {name:"家庭医生",url:"https://www.familydoctor.com.cn/"},
      {name:"寻医问药",url:"https://www.xywy.com/"},
      {name:"平安好医生",url:"https://www.jk.cn/"},
      {name:"健康界",url:"https://www.cn-healthcare.com/"},
      {name:"华医网",url:"https://www.91huayi.com/"}
    ],
    "医疗": [
      {name:"挂号",url:"https://www.guahao.com/"},
      {name:"阿里健康",url:"https://www.alihealth.com/"},
      {name:"丁香园",url:"https://www.dxy.cn/"},
      {name:"医脉通",url:"https://www.medlive.cn/"},
      {name:"中国医药网",url:"https://www.pharmnet.com.cn/"}
    ],
    "二手": [
      {name:"闲鱼",url:"https://www.goofish.com/"},
      {name:"转转",url:"https://www.zhuanzhuan.com/"},
      {name:"爱回收",url:"https://www.aihuishou.com/"},
      {name:"返利",url:"https://www.fanli.com/"},
      {name:"一淘",url:"https://www.etao.com/"}
    ],
    "折扣": [
      {name:"唯品会",url:"https://www.vip.com/"},
      {name:"网易严选",url:"https://you.163.com/"},
      {name:"蘑菇街",url:"https://www.mogu.com/"},
      {name:"有品",url:"https://www.xiaomiyoupin.com/"},
      {name:"国美",url:"https://www.gome.com.cn/"},
      {name:"微店",url:"https://www.weidian.com/"}
    ],
    "海淘": [
      {name:"亚马逊",url:"https://www.amazon.cn/"},
      {name:"AliExpress",url:"https://www.aliexpress.com/"},
      {name:"Spotify",url:"https://www.spotify.com/"},
      {name:"DuckDuckGo",url:"https://duckduckgo.com/"},
      {name:"Ecosia",url:"https://www.ecosia.org/"}
    ],
    "百度": [
      {name:"百度翻译",url:"https://fanyi.baidu.com/"},
      {name:"百度文库",url:"https://wenku.baidu.com/"},
      {name:"百家号",url:"https://baijiahao.baidu.com/"},
      {name:"百度智能云",url:"https://cloud.baidu.com/"}
    ],
    "设计": [
      {name:"站酷",url:"https://www.zcool.com.cn/"},
      {name:"花瓣",url:"https://huaban.com/"},
      {name:"视觉中国",url:"https://www.vcg.com/"},
      {name:"包图网",url:"https://www.ibaotu.com/"},
      {name:"千图网",url:"https://www.58pic.com/"},
      {name:"千库网",url:"https://www.588ku.com/"},
      {name:"摄图网",url:"https://www.699pic.com/"},
      {name:"昵图",url:"https://www.nipic.com/"}
    ],
    "摄影": [
      {name:"美图秀秀",url:"https://www.meitu.com/"},
      {name:"剪映",url:"https://www.capcut.cn/"},
      {name:"图虫",url:"https://tuchong.com/"},
      {name:"蜂鸟",url:"https://www.fengniao.com/"},
      {name:"堆糖",url:"https://www.duitang.com/"},
      {name:"瑞丽",url:"https://www.rayli.com.cn/"},
      {name:"无忌",url:"https://www.xitek.com/"},
      {name:"在线PS",url:"https://www.uupoop.com/"}
    ],
    "手机": [
      {name:"华为",url:"https://consumer.huawei.com/cn/"},
      {name:"小米",url:"https://www.mi.com/"},
      {name:"OPPO",url:"https://www.oppo.com/"},
      {name:"vivo",url:"https://www.vivo.com/"},
      {name:"Apple",url:"https://www.apple.com.cn/"},
      {name:"三星",url:"https://www.samsung.com/cn/"}
    ],
    "系统": [
      {name:"微软",url:"https://www.microsoft.com/"},
      {name:"360",url:"https://www.360.cn/"},
      {name:"搜狗输入法",url:"https://pinyin.sogou.com/"},
      {name:"迅雷",url:"https://www.xunlei.com/"},
      {name:"腾讯电脑管家",url:"https://guanjia.qq.com/"},
      {name:"鲁大师",url:"https://www.ludashi.com/"},
      {name:"7-Zip",url:"https://www.7-zip.org/"},
      {name:"Bandizip",url:"https://www.bandisoft.com/bandizip/"},
      {name:"格式工厂",url:"http://www.pcfreetime.com/"},
      {name:"比特彗星",url:"https://www.bitcomet.com/"},
      {name:"草料二维码",url:"https://cli.im/"},
      {name:"APKPure",url:"https://apkpure.com/"}
    ],
    "教育": [
      {name:"学信",url:"https://www.chsi.com.cn/"},
      {name:"研招网",url:"https://yz.chsi.com.cn/"},
      {name:"中公",url:"https://www.offcn.com/"},
      {name:"华图教育",url:"https://www.huatu.com/"},
      {name:"新东方在线",url:"https://www.koolearn.com/"},
      {name:"好未来",url:"https://www.100tal.com/"}
    ],
    "课教": [
      {name:"作业帮",url:"https://www.zuoyebang.com/"},
      {name:"猿辅导",url:"https://www.yuanfudao.com/"},
      {name:"学而思",url:"https://www.xueersi.com/"},
      {name:"高途",url:"https://www.gaotu.cn/"},
      {name:"腾讯课堂",url:"https://ke.qq.com/"},
      {name:"考研帮",url:"https://www.kaoyan.com/"},
      {name:"高考网",url:"https://www.gaokao.com/"}
    ],
    "网课": [
      {name:"中国大学MOOC",url:"https://www.icourse163.org/"},
      {name:"网易云课堂",url:"https://study.163.com/"},
      {name:"得到",url:"https://www.dedao.cn/"},
      {name:"学堂在线",url:"https://www.xuetangx.com/"},
      {name:"网易公开课",url:"https://open.163.com/"},
      {name:"Coursera",url:"https://www.coursera.org/"}
    ],
    "考证": [
      {name:"百词斩",url:"https://www.baicizhan.com/"},
      {name:"粉笔",url:"https://www.fenbi.com/"},
      {name:"扇贝",url:"https://www.shanbay.com/"},
      {name:"沪江",url:"https://www.hujiang.com/"}
    ],
    "通信": [
      {name:"中国移动",url:"https://www.10086.cn/"},
      {name:"中国联通",url:"https://www.10010.com/"},
      {name:"中国电信",url:"https://www.189.cn/"}
    ],
    "政府": [
      {name:"中国政府网",url:"https://www.gov.cn/"},
      {name:"国家税务总局",url:"https://www.chinatax.gov.cn/"},
      {name:"国家企业信用",url:"https://www.gsxt.gov.cn/"},
      {name:"教育部",url:"https://www.moe.gov.cn/"},
      {name:"统计局",url:"https://www.stats.gov.cn/"},
      {name:"信用中国",url:"https://www.creditchina.gov.cn/"},
      {name:"邮政局",url:"https://www.spb.gov.cn/"},
      {name:"中国青年",url:"https://www.youth.cn/"}
    ],
    "企查": [
      {name:"天眼查",url:"https://www.tianyancha.com/"},
      {name:"企查查",url:"https://www.qichacha.com/"},
      {name:"12321",url:"https://www.12321.cn/"}
    ]
  },
  en: {
    "Search": [
      {name:"Google",url:"https://www.google.com/"},
      {name:"Bing",url:"https://www.bing.com/"},
      {name:"Yahoo",url:"https://www.yahoo.com/"},
      {name:"DuckDuckGo",url:"https://duckduckgo.com/"},
      {name:"Yandex",url:"https://yandex.com/"},
      {name:"Ecosia",url:"https://www.ecosia.org/"},
      {name:"StartPage",url:"https://www.startpage.com/"},
      {name:"Brave Search",url:"https://search.brave.com/"}
    ],
    "Video": [
      {name:"YouTube",url:"https://www.youtube.com/"},
      {name:"TikTok",url:"https://www.tiktok.com/"},
      {name:"Twitch",url:"https://www.twitch.tv/"},
      {name:"Dailymotion",url:"https://www.dailymotion.com/"},
      {name:"Vimeo",url:"https://vimeo.com/"},
      {name:"Rumble",url:"https://rumble.com/"}
    ],
    "Social": [
      {name:"Facebook",url:"https://www.facebook.com/"},
      {name:"Instagram",url:"https://www.instagram.com/"},
      {name:"X (Twitter)",url:"https://x.com/"},
      {name:"Pinterest",url:"https://www.pinterest.com/"},
      {name:"Snapchat",url:"https://www.snapchat.com/"},
      {name:"Threads",url:"https://www.threads.net/"},
      {name:"Bluesky",url:"https://bsky.app/"},
      {name:"Mastodon",url:"https://mastodon.social/"}
    ],
    "Shop": [
      {name:"Amazon",url:"https://www.amazon.com/"},
      {name:"eBay",url:"https://www.ebay.com/"},
      {name:"Walmart",url:"https://www.walmart.com/"},
      {name:"Etsy",url:"https://www.etsy.com/"},
      {name:"Best Buy",url:"https://www.bestbuy.com/"},
      {name:"Target",url:"https://www.target.com/"},
      {name:"Costco",url:"https://www.costco.com/"},
      {name:"Home Depot",url:"https://www.homedepot.com/"},
      {name:"Newegg",url:"https://www.newegg.com/"},
      {name:"Amazon UK",url:"https://www.amazon.co.uk/"},
      {name:"Amazon India",url:"https://www.amazon.in/"}
    ],
    "AI": [
      {name:"ChatGPT",url:"https://chat.openai.com/"},
      {name:"Claude",url:"https://claude.ai/"},
      {name:"DeepSeek",url:"https://www.deepseek.com/"},
      {name:"Gemini",url:"https://gemini.google.com/"},
      {name:"Perplexity",url:"https://www.perplexity.ai/"},
      {name:"Grok",url:"https://grok.com/"},
      {name:"Poe",url:"https://poe.com/"}
    ],
    "Stream": [
      {name:"Netflix",url:"https://www.netflix.com/"},
      {name:"Disney+",url:"https://www.disneyplus.com/"},
      {name:"Amazon Prime",url:"https://www.primevideo.com/"},
      {name:"Hulu",url:"https://www.hulu.com/"},
      {name:"HBO Max",url:"https://www.max.com/"},
      {name:"Apple TV+",url:"https://tv.apple.com/"},
      {name:"Peacock",url:"https://www.peacocktv.com/"},
      {name:"Paramount+",url:"https://www.paramountplus.com/"},
      {name:"Plex",url:"https://www.plex.tv/"},
      {name:"Tubi",url:"https://tubitv.com/"},
      {name:"ITVX",url:"https://www.itv.com/"},
      {name:"Crunchyroll",url:"https://www.crunchyroll.com/"}
    ],
    "Chat": [
      {name:"WhatsApp",url:"https://www.whatsapp.com/"},
      {name:"Telegram",url:"https://telegram.org/"},
      {name:"Discord",url:"https://discord.com/"},
      {name:"Messenger",url:"https://www.messenger.com/"},
      {name:"Zoom",url:"https://zoom.us/"},
      {name:"Signal",url:"https://signal.org/"},
      {name:"Slack",url:"https://slack.com/"},
      {name:"WeChat",url:"https://www.wechat.com/"},
      {name:"Line",url:"https://line.me/"},
      {name:"Viber",url:"https://www.viber.com/"},
      {name:"TeamViewer",url:"https://www.teamviewer.com/"},
      {name:"Skype",url:"https://www.skype.com/"}
    ],
    "News": [
      {name:"BBC",url:"https://www.bbc.com/"},
      {name:"CNN",url:"https://www.cnn.com/"},
      {name:"NY Times",url:"https://www.nytimes.com/"},
      {name:"MSN",url:"https://www.msn.com/"},
      {name:"The Guardian",url:"https://www.theguardian.com/"},
      {name:"Washington Post",url:"https://www.washingtonpost.com/"},
      {name:"WSJ",url:"https://www.wsj.com/"},
      {name:"NBC News",url:"https://www.nbcnews.com/"},
      {name:"CBS News",url:"https://www.cbsnews.com/"},
      {name:"ABC News",url:"https://abcnews.go.com/"},
      {name:"NPR",url:"https://www.npr.org/"},
      {name:"USA Today",url:"https://www.usatoday.com/"},
      {name:"Politico",url:"https://www.politico.com/"},
      {name:"HuffPost",url:"https://www.huffpost.com/"},
      {name:"Sky News",url:"https://news.sky.com/"},
      {name:"Axios",url:"https://www.axios.com/"},
      {name:"CBC",url:"https://www.cbc.ca/"},
      {name:"BBC iPlayer",url:"https://www.bbc.co.uk/iplayer/"},
      {name:"Daily Mail",url:"https://www.dailymail.co.uk/"},
      {name:"LA Times",url:"https://www.latimes.com/"},
      {name:"Associated Press",url:"https://apnews.com/"},
      {name:"Time",url:"https://time.com/"},
      {name:"Insider",url:"https://www.insider.com/"},
      {name:"New York Post",url:"https://nypost.com/"},
      {name:"Al Jazeera",url:"https://www.aljazeera.com/"},
      {name:"The Atlantic",url:"https://www.theatlantic.com/"},
      {name:"Newsweek",url:"https://www.newsweek.com/"},
      {name:"The Independent",url:"https://www.independent.co.uk/"},
      {name:"The Telegraph",url:"https://www.telegraph.co.uk/"},
      {name:"The Sun",url:"https://www.thesun.co.uk/"},
      {name:"The Times",url:"https://www.thetimes.com/"},
      {name:"The New Yorker",url:"https://www.newyorker.com/"},
      {name:"Metro",url:"https://metro.co.uk/"},
      {name:"Substack",url:"https://substack.com/"}
    ],
    "Media": [
      {name:"Billboard",url:"https://www.billboard.com/"},
      {name:"People",url:"https://people.com/"},
      {name:"TMZ",url:"https://www.tmz.com/"},
      {name:"Rolling Stone",url:"https://www.rollingstone.com/"},
      {name:"Variety",url:"https://variety.com/"},
      {name:"The Hollywood Reporter",url:"https://www.hollywoodreporter.com/"},
      {name:"Deadline",url:"https://deadline.com/"},
      {name:"Pitchfork",url:"https://pitchfork.com/"},
      {name:"Know Your Meme",url:"https://knowyourmeme.com/"}
    ],
    "Tech": [
      {name:"TechCrunch",url:"https://techcrunch.com/"},
      {name:"The Verge",url:"https://www.theverge.com/"},
      {name:"Wired",url:"https://www.wired.com/"},
      {name:"Ars Technica",url:"https://arstechnica.com/"},
      {name:"CNET",url:"https://www.cnet.com/"},
      {name:"Engadget",url:"https://www.engadget.com/"},
      {name:"Gizmodo",url:"https://gizmodo.com/"},
      {name:"Slashdot",url:"https://slashdot.org/"},
      {name:"ZDNet",url:"https://www.zdnet.com/"}
    ],
    "Geek": [
      {name:"Hacker News",url:"https://news.ycombinator.com/"},
      {name:"Product Hunt",url:"https://www.producthunt.com/"},
      {name:"Mashable",url:"https://mashable.com/"},
      {name:"Lifehacker",url:"https://lifehacker.com/"}
    ],
    "Biz": [
      {name:"Bloomberg",url:"https://www.bloomberg.com/"},
      {name:"Forbes",url:"https://www.forbes.com/"},
      {name:"Business Insider",url:"https://www.businessinsider.com/"},
      {name:"Fortune",url:"https://fortune.com/"}
    ],
    "Music": [
      {name:"Spotify",url:"https://www.spotify.com/"},
      {name:"Apple Music",url:"https://music.apple.com/"},
      {name:"SoundCloud",url:"https://soundcloud.com/"},
      {name:"Bandcamp",url:"https://bandcamp.com/"},
      {name:"Genius",url:"https://genius.com/"},
      {name:"Discogs",url:"https://www.discogs.com/"}
    ],
    "Forum": [
      {name:"Reddit",url:"https://www.reddit.com/"},
      {name:"Quora",url:"https://www.quora.com/"},
      {name:"Medium",url:"https://medium.com/"},
      {name:"BuzzFeed",url:"https://www.buzzfeed.com/"},
      {name:"Tumblr",url:"https://www.tumblr.com/"},
      {name:"Imgur",url:"https://imgur.com/"},
      {name:"Flickr",url:"https://www.flickr.com/"},
      {name:"9GAG",url:"https://9gag.com/"}
    ],
    "Games": [
      {name:"Roblox",url:"https://www.roblox.com/"},
      {name:"Steam",url:"https://store.steampowered.com/"},
      {name:"Epic Games",url:"https://store.epicgames.com/"},
      {name:"Minecraft",url:"https://www.minecraft.net/"},
      {name:"Nintendo",url:"https://www.nintendo.com/"},
      {name:"PlayStation",url:"https://www.playstation.com/"},
      {name:"Xbox",url:"https://www.xbox.com/"},
      {name:"Riot Games",url:"https://www.riotgames.com/"},
      {name:"GameSpot",url:"https://www.gamespot.com/"},
      {name:"Polygon",url:"https://www.polygon.com/"},
      {name:"Kotaku",url:"https://kotaku.com/"},
      {name:"MyAnimeList",url:"https://myanimelist.net/"},
      {name:"Bored Panda",url:"https://www.boredpanda.com/"},
      {name:"Cricbuzz",url:"https://www.cricbuzz.com/"},
      {name:"GDNet",url:"https://www.gdnet.com/"},
      {name:"Steam Community",url:"https://steamcommunity.com/"}
    ],
    "Movie": [
      {name:"IMDb",url:"https://www.imdb.com/"},
      {name:"Rotten Tomatoes",url:"https://www.rottentomatoes.com/"},
      {name:"IGN",url:"https://www.ign.com/"},
      {name:"Metacritic",url:"https://www.metacritic.com/"},
      {name:"Fandom",url:"https://www.fandom.com/"},
      {name:"Letterboxd",url:"https://letterboxd.com/"}
    ],
    "Sports": [
      {name:"ESPN",url:"https://www.espn.com/"},
      {name:"NBA",url:"https://www.nba.com/"},
      {name:"NFL",url:"https://www.nfl.com/"},
      {name:"MLB",url:"https://www.mlb.com/"},
      {name:"NHL",url:"https://www.nhl.com/"},
      {name:"Bleacher Report",url:"https://bleacherreport.com/"},
      {name:"UFC",url:"https://www.ufc.com/"},
      {name:"ESPN+",url:"https://plus.espn.com/"},
      {name:"DAZN",url:"https://www.dazn.com/"}
    ],
    "Finance": [
      {name:"Yahoo Finance",url:"https://finance.yahoo.com/"},
      {name:"Reuters",url:"https://www.reuters.com/"},
      {name:"Investing.com",url:"https://www.investing.com/"},
      {name:"CNBC",url:"https://www.cnbc.com/"},
      {name:"MarketWatch",url:"https://www.marketwatch.com/"},
      {name:"Financial Times",url:"https://www.ft.com/"},
      {name:"The Economist",url:"https://www.economist.com/"},
      {name:"Seeking Alpha",url:"https://seekingalpha.com/"}
    ],
    "Banks": [
      {name:"Chase",url:"https://www.chase.com/"},
      {name:"Bank of America",url:"https://www.bankofamerica.com/"},
      {name:"Wells Fargo",url:"https://www.wellsfargo.com/"},
      {name:"Capital One",url:"https://www.capitalone.com/"},
      {name:"American Express",url:"https://www.americanexpress.com/"},
      {name:"Citibank",url:"https://www.citi.com/"},
      {name:"HSBC",url:"https://www.hsbc.com/"},
      {name:"US Bank",url:"https://www.usbank.com/"},
      {name:"Barclays",url:"https://www.barclays.com/"}
    ],
    "Invest": [
      {name:"Vanguard",url:"https://investor.vanguard.com/"},
      {name:"Fidelity",url:"https://www.fidelity.com/"},
      {name:"Charles Schwab",url:"https://www.schwab.com/"},
      {name:"JPMorgan Chase",url:"https://www.jpmorganchase.com/"},
      {name:"Goldman Sachs",url:"https://www.goldmansachs.com/"}
    ],
    "Pay": [
      {name:"PayPal",url:"https://www.paypal.com/"},
      {name:"Visa",url:"https://www.visa.com/"},
      {name:"Stripe",url:"https://stripe.com/"},
      {name:"Mastercard",url:"https://www.mastercard.com/"},
      {name:"Venmo",url:"https://venmo.com/"},
      {name:"Revolut",url:"https://www.revolut.com/"},
      {name:"Wise",url:"https://wise.com/"}
    ],
    "Crypto": [
      {name:"Binance",url:"https://www.binance.com/"},
      {name:"Coinbase",url:"https://www.coinbase.com/"},
      {name:"Robinhood",url:"https://robinhood.com/"},
      {name:"CoinMarketCap",url:"https://coinmarketcap.com/"},
      {name:"CoinGecko",url:"https://www.coingecko.com/"}
    ],
    "Health": [
      {name:"WebMD",url:"https://www.webmd.com/"},
      {name:"Mayo Clinic",url:"https://www.mayoclinic.org/"},
      {name:"Healthline",url:"https://www.healthline.com/"},
      {name:"NHS",url:"https://www.nhs.uk/"},
      {name:"NIH",url:"https://www.nih.gov/"},
      {name:"CDC",url:"https://www.cdc.gov/"},
      {name:"WHO",url:"https://www.who.int/"},
      {name:"Medscape",url:"https://www.medscape.com/"},
      {name:"Cleveland Clinic",url:"https://my.clevelandclinic.org/"},
      {name:"Drugs.com",url:"https://www.drugs.com/"},
      {name:"Verywell Health",url:"https://www.verywellhealth.com/"}
    ],
    "Wellness": [
      {name:"Strava",url:"https://www.strava.com/"},
      {name:"MyFitnessPal",url:"https://www.myfitnesspal.com/"},
      {name:"Calm",url:"https://www.calm.com/"},
      {name:"Headspace",url:"https://www.headspace.com/"}
    ],
    "Travel": [
      {name:"Airbnb",url:"https://www.airbnb.com/"},
      {name:"TripAdvisor",url:"https://www.tripadvisor.com/"},
      {name:"Expedia",url:"https://www.expedia.com/"},
      {name:"Hotels.com",url:"https://www.hotels.com/"},
      {name:"Kayak",url:"https://www.kayak.com/"},
      {name:"Agoda",url:"https://www.agoda.com/"},
      {name:"Klook",url:"https://www.klook.com/"},
      {name:"Priceline",url:"https://www.priceline.com/"},
      {name:"Marriott",url:"https://www.marriott.com/"},
      {name:"Hilton",url:"https://www.hilton.com/"},
      {name:"Orbitz",url:"https://www.orbitz.com/"},
      {name:"AllTrails",url:"https://www.alltrails.com/"},
      {name:"Atlas Obscura",url:"https://www.atlasobscura.com/"}
    ],
    "Food": [
      {name:"Uber Eats",url:"https://www.ubereats.com/"},
      {name:"DoorDash",url:"https://www.doordash.com/"},
      {name:"Instacart",url:"https://www.instacart.com/"},
      {name:"Grubhub",url:"https://www.grubhub.com/"},
      {name:"Just Eat",url:"https://www.just-eat.co.uk/"},
      {name:"Deliveroo",url:"https://deliveroo.co.uk/"},
      {name:"Zomato",url:"https://www.zomato.com/"},
      {name:"Swiggy",url:"https://www.swiggy.com/"},
      {name:"OpenTable",url:"https://www.opentable.com/"},
      {name:"Grab",url:"https://www.grab.com/"},
      {name:"Gojek",url:"https://www.gojek.com/"},
      {name:"Food Network",url:"https://www.foodnetwork.com/"},
      {name:"Allrecipes",url:"https://www.allrecipes.com/"},
      {name:"Tasty",url:"https://tasty.co/"},
      {name:"Epicurious",url:"https://www.epicurious.com/"}
    ],
    "Life": [
      {name:"Yelp",url:"https://www.yelp.com/"},
      {name:"Craigslist",url:"https://www.craigslist.org/"},
      {name:"Zillow",url:"https://www.zillow.com/"},
      {name:"OnlyFans",url:"https://onlyfans.com/"},
      {name:"Patreon",url:"https://www.patreon.com/"},
      {name:"Kickstarter",url:"https://www.kickstarter.com/"},
      {name:"Groupon",url:"https://www.groupon.com/"},
      {name:"GoFundMe",url:"https://www.gofundme.com/"},
      {name:"Change.org",url:"https://www.change.org/"},
      {name:"Realtor",url:"https://www.realtor.com/"},
      {name:"Redfin",url:"https://www.redfin.com/"},
      {name:"Nextdoor",url:"https://nextdoor.com/"},
      {name:"Meetup",url:"https://www.meetup.com/"},
      {name:"OfferUp",url:"https://offerup.com/"},
      {name:"CVS",url:"https://www.cvs.com/"},
      {name:"Walgreens",url:"https://www.walgreens.com/"},
      {name:"Petfinder",url:"https://www.petfinder.com/"},
      {name:"Yellow Pages",url:"https://www.yellowpages.com/"}
    ],
    "Fashion": [
      {name:"Nike",url:"https://www.nike.com/"},
      {name:"Adidas",url:"https://www.adidas.com/"},
      {name:"H&M",url:"https://www.hm.com/"},
      {name:"Zara",url:"https://www.zara.com/"},
      {name:"Uniqlo",url:"https://www.uniqlo.com/"},
      {name:"SHEIN",url:"https://www.shein.com/"},
      {name:"ASOS",url:"https://www.asos.com/"},
      {name:"Lululemon",url:"https://www.lululemon.com/"},
      {name:"Farfetch",url:"https://www.farfetch.com/"},
      {name:"Zalando",url:"https://www.zalando.com/"},
      {name:"Ssense",url:"https://www.ssense.com/"},
      {name:"Net-a-Porter",url:"https://www.net-a-porter.com/"}
    ],
    "Jobs": [
      {name:"Indeed",url:"https://www.indeed.com/"},
      {name:"Glassdoor",url:"https://www.glassdoor.com/"},
      {name:"Upwork",url:"https://www.upwork.com/"},
      {name:"Fiverr",url:"https://www.fiverr.com/"},
      {name:"Monster",url:"https://www.monster.com/"},
      {name:"ZipRecruiter",url:"https://www.ziprecruiter.com/"}
    ],
    "Learn": [
      {name:"Duolingo",url:"https://www.duolingo.com/"},
      {name:"Coursera",url:"https://www.coursera.org/"},
      {name:"Udemy",url:"https://www.udemy.com/"},
      {name:"TED",url:"https://www.ted.com/"},
      {name:"Quizlet",url:"https://quizlet.com/"},
      {name:"edX",url:"https://www.edx.org/"},
      {name:"Codecademy",url:"https://www.codecademy.com/"},
      {name:"freeCodeCamp",url:"https://www.freecodecamp.org/"},
      {name:"Chegg",url:"https://www.chegg.com/"},
      {name:"Skillshare",url:"https://www.skillshare.com/"},
      {name:"Brilliant",url:"https://brilliant.org/"},
      {name:"MasterClass",url:"https://www.masterclass.com/"},
      {name:"DataCamp",url:"https://www.datacamp.com/"},
      {name:"Pluralsight",url:"https://www.pluralsight.com/"},
      {name:"MIT OCW",url:"https://ocw.mit.edu/"}
    ],
    "Wiki": [
      {name:"Wikipedia",url:"https://www.wikipedia.org/"},
      {name:"Goodreads",url:"https://www.goodreads.com/"},
      {name:"wikiHow",url:"https://www.wikihow.com/"},
      {name:"HowStuffWorks",url:"https://www.howstuffworks.com/"},
      {name:"ResearchGate",url:"https://www.researchgate.net/"},
      {name:"Scribd",url:"https://www.scribd.com/"},
      {name:"Thesaurus.com",url:"https://www.thesaurus.com/"},
      {name:"WolframAlpha",url:"https://www.wolframalpha.com/"}
    ],
    "PM": [
      {name:"Notion",url:"https://www.notion.so/"},
      {name:"Google Docs",url:"https://docs.google.com/"},
      {name:"Google Calendar",url:"https://calendar.google.com/"},
      {name:"Grammarly",url:"https://www.grammarly.com/"},
      {name:"Evernote",url:"https://evernote.com/"},
      {name:"Miro",url:"https://miro.com/"},
      {name:"Jira",url:"https://www.atlassian.com/software/jira"},
      {name:"Asana",url:"https://asana.com/"},
      {name:"Airtable",url:"https://airtable.com/"},
      {name:"Trello",url:"https://trello.com/"},
      {name:"Confluence",url:"https://www.atlassian.com/software/confluence"},
      {name:"ClickUp",url:"https://clickup.com/"},
      {name:"Todoist",url:"https://todoist.com/"},
      {name:"Calendly",url:"https://calendly.com/"}
    ],
    "Storage": [
      {name:"Google Drive",url:"https://drive.google.com/"},
      {name:"Dropbox",url:"https://www.dropbox.com/"},
      {name:"OneDrive",url:"https://onedrive.live.com/"},
      {name:"iCloud",url:"https://www.icloud.com/"},
      {name:"Box",url:"https://www.box.com/"},
      {name:"Mega",url:"https://mega.io/"},
      {name:"TeraBox",url:"https://www.terabox.com/"}
    ],
    "Office": [
      {name:"Microsoft 365",url:"https://www.office.com/"},
      {name:"Microsoft Teams",url:"https://www.microsoft.com/microsoft-teams/"}
    ],
    "Email": [
      {name:"Gmail",url:"https://mail.google.com/"},
      {name:"Outlook",url:"https://outlook.live.com/"},
      {name:"Proton",url:"https://proton.me/"}
    ],
    "Cloud": [
      {name:"AWS",url:"https://aws.amazon.com/"},
      {name:"Azure",url:"https://azure.microsoft.com/"},
      {name:"Google Cloud",url:"https://cloud.google.com/"},
      {name:"Cloudflare",url:"https://www.cloudflare.com/"},
      {name:"Vercel",url:"https://vercel.com/"},
      {name:"DigitalOcean",url:"https://www.digitalocean.com/"},
      {name:"Heroku",url:"https://www.heroku.com/"},
      {name:"Netlify",url:"https://www.netlify.com/"},
      {name:"Linode",url:"https://www.linode.com/"},
      {name:"Hetzner",url:"https://www.hetzner.com/"},
      {name:"OVHcloud",url:"https://www.ovhcloud.com/"},
      {name:"Fastly",url:"https://www.fastly.com/"},
      {name:"Vultr",url:"https://www.vultr.com/"}
    ],
    "Dev": [
      {name:"GitHub",url:"https://github.com/"},
      {name:"GitLab",url:"https://gitlab.com/"},
      {name:"Docker",url:"https://www.docker.com/"},
      {name:"Atlassian",url:"https://www.atlassian.com/"},
      {name:"Bitbucket",url:"https://bitbucket.org/"},
      {name:"Kubernetes",url:"https://kubernetes.io/"},
      {name:"Jenkins",url:"https://www.jenkins.io/"},
      {name:"Terraform",url:"https://www.terraform.io/"},
      {name:"Ansible",url:"https://www.ansible.com/"},
      {name:"CircleCI",url:"https://circleci.com/"},
      {name:"Docker Hub",url:"https://hub.docker.com/"},
      {name:"npm",url:"https://www.npmjs.com/"},
      {name:"Python",url:"https://www.python.org/"},
      {name:"Replit",url:"https://replit.com/"},
      {name:"CodePen",url:"https://codepen.io/"},
      {name:"Postman",url:"https://www.postman.com/"},
      {name:"Dev.to",url:"https://dev.to/"},
      {name:"SourceForge",url:"https://sourceforge.net/"},
      {name:"PyPI",url:"https://pypi.org/"},
      {name:"JSFiddle",url:"https://jsfiddle.net/"},
      {name:"MDN Web Docs",url:"https://developer.mozilla.org/"},
      {name:"W3Schools",url:"https://www.w3schools.com/"}
    ],
    "Tools": [
      {name:"Git",url:"https://git-scm.com/"},
      {name:"Node.js",url:"https://nodejs.org/"},
      {name:"VS Code",url:"https://code.visualstudio.com/"},
      {name:"Mozilla",url:"https://www.mozilla.org/"},
      {name:"Notepad++",url:"https://notepad-plus-plus.org/"},
      {name:"Sublime",url:"https://www.sublimetext.com/"}
    ],
    "Design": [
      {name:"Adobe",url:"https://www.adobe.com/"},
      {name:"Canva",url:"https://www.canva.com/"},
      {name:"Figma",url:"https://www.figma.com/"},
      {name:"Dribbble",url:"https://dribbble.com/"},
      {name:"Behance",url:"https://www.behance.net/"},
      {name:"CapCut",url:"https://www.capcut.com/"},
      {name:"Flaticon",url:"https://www.flaticon.com/"},
      {name:"Coolors",url:"https://coolors.co/"},
      {name:"Adobe Acrobat",url:"https://acrobat.adobe.com/"},
      {name:"Awwwards",url:"https://www.awwwards.com/"},
      {name:"IconFinder",url:"https://www.iconfinder.com/"},
      {name:"Remove.bg",url:"https://www.remove.bg/"}
    ],
    "Photos": [
      {name:"Unsplash",url:"https://unsplash.com/"},
      {name:"Pexels",url:"https://www.pexels.com/"},
      {name:"Pixabay",url:"https://pixabay.com/"},
      {name:"Shutterstock",url:"https://www.shutterstock.com/"},
      {name:"GIPHY",url:"https://giphy.com/"},
      {name:"DeviantArt",url:"https://www.deviantart.com/"},
      {name:"500px",url:"https://500px.com/"}
    ],
    "Hardware": [
      {name:"Apple",url:"https://www.apple.com/"},
      {name:"Samsung",url:"https://www.samsung.com/"},
      {name:"Nvidia",url:"https://www.nvidia.com/"},
      {name:"Intel",url:"https://www.intel.com/"},
      {name:"AMD",url:"https://www.amd.com/"},
      {name:"Dell",url:"https://www.dell.com/"},
      {name:"HP",url:"https://www.hp.com/"},
      {name:"IBM",url:"https://www.ibm.com/"},
      {name:"Cisco",url:"https://www.cisco.com/"},
      {name:"Oracle",url:"https://www.oracle.com/"},
      {name:"iFixit",url:"https://www.ifixit.com/"}
    ],
    "Hosting": [
      {name:"GoDaddy",url:"https://www.godaddy.com/"},
      {name:"Shopify",url:"https://www.shopify.com/"},
      {name:"WordPress",url:"https://wordpress.com/"},
      {name:"Salesforce",url:"https://www.salesforce.com/"},
      {name:"Wix",url:"https://www.wix.com/"},
      {name:"Squarespace",url:"https://www.squarespace.com/"},
      {name:"Webflow",url:"https://webflow.com/"},
      {name:"Namecheap",url:"https://www.namecheap.com/"},
      {name:"DreamHost",url:"https://www.dreamhost.com/"},
      {name:"Tailwind CSS",url:"https://tailwindcss.com/"}
    ],
    "Models": [
      {name:"OpenAI",url:"https://openai.com/"},
      {name:"Anthropic",url:"https://www.anthropic.com/"},
      {name:"Hugging Face",url:"https://huggingface.co/"},
      {name:"Mistral AI",url:"https://mistral.ai/"},
      {name:"Cohere",url:"https://cohere.com/"},
      {name:"Character.ai",url:"https://character.ai/"},
      {name:"Replicate",url:"https://replicate.com/"},
      {name:"OpenRouter",url:"https://openrouter.ai/"}
    ],
    "Deals": [
      {name:"Temu",url:"https://www.temu.com/"},
      {name:"Alibaba",url:"https://www.alibaba.com/"},
      {name:"AliExpress",url:"https://www.aliexpress.com/"},
      {name:"Shopee",url:"https://shopee.com/"},
      {name:"Flipkart",url:"https://www.flipkart.com/"},
      {name:"Rakuten",url:"https://www.rakuten.com/"},
      {name:"Mercari",url:"https://www.mercari.com/"},
      {name:"Wayfair",url:"https://www.wayfair.com/"},
      {name:"Lowe's",url:"https://www.lowes.com/"},
      {name:"Macy's",url:"https://www.macys.com/"},
      {name:"StockX",url:"https://stockx.com/"},
      {name:"Poshmark",url:"https://poshmark.com/"},
      {name:"Wish",url:"https://www.wish.com/"}
    ],
    "Cars": [
      {name:"Tesla",url:"https://www.tesla.com/"},
      {name:"Toyota",url:"https://www.toyota.com/"},
      {name:"CarMax",url:"https://www.carmax.com/"},
      {name:"AutoTrader",url:"https://www.autotrader.com/"},
      {name:"BMW",url:"https://www.bmw.com/"},
      {name:"Ford",url:"https://www.ford.com/"},
      {name:"Edmunds",url:"https://www.edmunds.com/"},
      {name:"Cars.com",url:"https://www.cars.com/"},
      {name:"Carvana",url:"https://www.carvana.com/"},
      {name:"Kelley Blue Book",url:"https://www.kbb.com/"},
      {name:"Mercedes-Benz",url:"https://www.mercedes-benz.com/"}
    ],
    "Transit": [
      {name:"Uber",url:"https://www.uber.com/"},
      {name:"Lyft",url:"https://www.lyft.com/"},
      {name:"Skyscanner",url:"https://www.skyscanner.com/"},
      {name:"GetYourGuide",url:"https://www.getyourguide.com/"},
      {name:"Viator",url:"https://www.viator.com/"},
      {name:"Omio",url:"https://www.omio.com/"},
      {name:"Rome2Rio",url:"https://www.rome2rio.com/"},
      {name:"Bolt",url:"https://bolt.eu/"},
      {name:"Hostelworld",url:"https://www.hostelworld.com/"}
    ],
    "Maps": [
      {name:"Google Maps",url:"https://maps.google.com/"},
      {name:"Waze",url:"https://www.waze.com/"}
    ],
    "Weather": [
      {name:"AccuWeather",url:"https://www.accuweather.com/"}
    ],
    "Ship": [
      {name:"DHL",url:"https://www.dhl.com/"},
      {name:"USPS",url:"https://www.usps.com/"}
    ],
    "Bus": [
      {name:"FlixBus",url:"https://www.flixbus.com/"},
      {name:"BlaBlaCar",url:"https://www.blablacar.com/"},
      {name:"Amtrak",url:"https://www.amtrak.com/"},
      {name:"Busbud",url:"https://www.busbud.com/"}
    ],
    "Air": [
      {name:"Delta",url:"https://www.delta.com/"},
      {name:"United",url:"https://www.united.com/"},
      {name:"American Airlines",url:"https://www.aa.com/"},
      {name:"Southwest",url:"https://www.southwest.com/"},
      {name:"Hertz",url:"https://www.hertz.com/"}
    ],
    "Security": [
      {name:"Brave",url:"https://brave.com/"},
      {name:"NordVPN",url:"https://nordvpn.com/"},
      {name:"Bitwarden",url:"https://bitwarden.com/"},
      {name:"LastPass",url:"https://www.lastpass.com/"}
    ],
    "Image": [
      {name:"Midjourney",url:"https://www.midjourney.com/"},
      {name:"Runway",url:"https://runwayml.com/"},
      {name:"Stability AI",url:"https://stability.ai/"},
      {name:"Leonardo AI",url:"https://leonardo.ai/"}
    ],
    "Money": [
      {name:"Credit Karma",url:"https://www.creditkarma.com/"},
      {name:"TradingView",url:"https://www.tradingview.com/"},
      {name:"NerdWallet",url:"https://www.nerdwallet.com/"},
      {name:"Morningstar",url:"https://www.morningstar.com/"},
      {name:"Mint",url:"https://mint.intuit.com/"}
    ]
  }
};
